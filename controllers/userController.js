...
export const editNickname = async (req, res) => {
    ...
    try {
        ...
        // 변경된 필드만 업데이트
        const affectedRows = await User.updateUser(userId, user.email, user.password, newNickname, user.profile_image);
        if (affectedRows === 0) {
            return res.status(404).json({ message: "사용자를 찾을 수 없습니다.", data: null });
        }
        ...
        console.log("닉네임 변경");
        return res.status(200).json({ message: "닉네임 변경 성공", data: req.session.user });
    } catch (err) {
        console.error("데이터베이스 오류:", err);
        return res.status(500).json({ message: "서버에 오류가 발생했습니다.", data: null });
    }
...
export const editProfileImage = async (req, res) => {
    ...
    try {
        ...
        // 변경된 필드만 업데이트
        const affectedRows = await User.updateUser(userId, user.email, user.password, user.nickname, newProfileImg);
        if (affectedRows === 0) {
            return res.status(404).json({ message: "사용자를 찾을 수 없습니다.", data: null });
        }
        ...
        return res.status(200).json({ message: "프로필 이미지 변경 성공", data: req.session.user });
    } catch (err) {
        console.error("데이터베이스 오류:", err);
        return res.status(500).json({ message: "서버에 오류가 발생했습니다.", data: null });
    }
